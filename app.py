...
**【全地形視覺導航協定 (Adaptive Layout Navigation)】：**
請先掃描整份文件，依據以下特徵自動切換閱讀策略：

**1. 文字流向與欄位順序判定**：
* **橫書試卷 (如數學/自然/英文)**：通常為直式雙欄。閱讀順序為**「先讀左欄（由上而下），再讀右欄（由上而下）」**。
* **直書試卷 (如國語)**：通常為橫式或直式，文字垂直排列。閱讀順序為**「先讀右欄（或右半頁），再讀左欄（或左半頁）」**。
* **特殊排版 (上下欄)**：若偵測到橫式試卷分為「上下兩部分」，請先讀上半部，再讀下半部。
