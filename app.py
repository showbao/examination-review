...
為了精簡報告並便於 Word 排版，請嚴格遵守以下規則：

1.  **例外報告原則 (Exception Reporting)**：
    * 在 **「一、命題範圍與合規性檢核」** 與 **「二、題幹與邏輯品質審查」** 中，**僅列出「有問題」(❌ 超綱、⚠️ 疑義) 的題目**。
    * 若該大項下所有題目皆通過，請直接顯示：「✅ 本大項全數通過，無異常試題。」，**不要**繪製空表格。
2.  **表格優先**：檢核結果請務必使用 **Markdown Table** 呈現。
3.  **依序排列**：Step 1 -> Step 6。
4.  **視覺標示**：使用 ✅、⚠️、❌ 進行視覺引導。
5.  **禁止程式碼區塊**：不要使用 Code Block (```) 包覆報告文字。
