    ...
    try:
        ...
        for m in all_models:
            if 'generateContent' in m.supported_generation_methods:
                ...
                # 權重計分 (Pro 分數最高，故 Auto 模式預設選 Pro)
                score = 0
...
with st.container():
    ...
    with col_dash_1:
        ...
        <div class="dashboard-card">
            ...
            <small>預設啟用：自動科目路由 (優先啟用 Pro 核心以確保深度邏輯審查)</small>
        </div>
        """, unsafe_allow_html=True)
    with col_dash_2:
...
1. **嚴禁開場白**：禁止輸出「依據剝皮測試標準，抽樣分析如下...」等引言，直接開始列出題目。
2. **禁止頁碼**：題目敘述中**嚴禁提及**「第x頁」、「第x頁之(3)」等頁碼資訊。
3. **強制分組與排序**：在 Step 2 與 Step 3，必須將同類燈號集中顯示。排序邏輯：**先列出所有 🟢(或優點)，再列出 🔴(或缺點/假素養)，最後列出 🟡(建議)**。
4. **強制換行**：**每一個**燈號項目都必須是獨立的一行 (使用 Markdown Bullet Point 語法 `- `)。禁止將多個題目擠在同一段。
...
### Step 2: 題幹與邏輯品質
* 檢查語意歧義、邏輯謬誤。
* **分組排序**：請先列出所有🟢優點，再列出🔴邏輯錯誤，最後列出🟡語意不清建議。
* **強制換行**：確保每一題都是獨立的一行。
* 輸出格式範例：
    * 🟢 **第 5 題**：選項邏輯清晰...
    * 🟡 **第 3 題**：題幹語意不清，建議修改...
