...
**【閱卷視覺導引 (Dynamic Visual Navigation)】：**
請先執行「版面結構分析」，再決定閱讀順序：
1.  **偵測排版模式**：
    * **雙欄模式 (Dual-Column)**：若試卷中間有明顯的「分隔線」或「垂直空白間隔」，且題目編號在左半邊到底後，才出現在右半邊頂端（例如左欄是1-10題，右欄是11-20題），請**務必採用「N字型閱讀法」**：先讀完左欄（由上而下），再讀右欄（由上而下）。
    * **單欄模式 (Single-Column)**：若題目是標準的由左至右、由上而下排列，請採用**「Z字型閱讀法」**（標準閱讀）。
2.  **混合排版處理**：若試卷上半部是單欄（如大題說明），下半部變成雙欄，請依區塊自動切換閱讀視線，確保題意完整，**嚴禁將左右欄文字混雜讀取**。
...
---
## Step 1: 命題範圍與形式檢查 (強制驗算模式)
**1. 配分與題號檢核 (Step-by-Step Verification)**：
* **請不要直接回答合規**，你必須在腦中執行以下計算，並將結果輸出：
    * **排版確認**：請第一句話先告訴我，你偵測到的是「單欄排版」還是「雙欄排版」。
    * **計算檢核**：針對每一大題，計算「題目數量 × 每題分數 = 該大題總分」。
    * **總分加總**：將所有大題總分相加，確認是否嚴格等於 100 分。
    * **題號檢查**：檢查題號是否連續。若為雙欄，請特別檢查「左欄最後一題」與「右欄第一題」是否連號。
* **請分組**：
    * ### 🟢 形式合規 (若驗算無誤，請註明：偵測為X欄排版、題號連續、總分經計算確為100分)
    * ### 🔴 形式錯誤 (若有錯誤，請具體列出：哪一大題題數算錯、跳號、或總分加總為多少)
...
## Step 2: 題幹與邏輯品質
* **請分組**：### 🟢 優良試題 、 ### 🔴 待改善試題 、 ### 🟡 建議確認。
* **誘答力檢核**：(針對選擇題) 選項是否具備合理的誘答性？是否出現刪去法即可作答的明顯瑕疵？
* **豁免應用**：若發現是非題的錯誤敘述是為了測驗觀念（答案為 X），**務必歸類於「🟢 優良試題」**。
