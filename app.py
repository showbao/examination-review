...
### 🟢 優良試題
(**強制清單**：每一題務必換行，使用列點符號 (*) 開頭；每一點都必須包含「題號 + 題目錨點 + 為何優良的具體原因」。)

### 🟡 待確認試題
(包含誘答力不足、邏輯瑕疵等問題，**強制清單**：每一題務必換行，使用列點符號 (*) 開頭，並具體說明原因。)

### 題幹完整性檢查
(檢查題幹資訊是否完整、是否缺乏必要條件、是否存在無法作答情形。)

### 選項與干擾品質
(檢查選項是否具有合理干擾性，是否存在明顯錯誤選項或選項長度差異過大。)

### 是否存在提示線索
(檢查題幹或選項是否提供過度提示，使學生可不經思考直接猜出答案。)
...
### 推理需求
(分析學生是否需要進行推論、判斷或解釋，而非直接記憶。)

### 跨概念整合
(檢查是否整合多個概念或學習單元。)

### 情境包裝程度
(判斷情境是否只是包裝知識，而非解題必要條件。)
