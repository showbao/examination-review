...
def get_model_obj(api_key, model_name):
    """取得 Gemini 模型物件"""
    genai.configure(api_key=api_key)
    return genai.GenerativeModel(model_name)

def get_best_flash_model(api_key):
    """獲取當前最好的 Flash 模型 (用於 metadata 提取與文科審查)"""
    genai.configure(api_key=api_key)
    try:
...
def get_best_pro_model(api_key):
    ...
    try:
        ...
        models.sort(key=lambda x: x.name, reverse=True) # 找最新的 Pro
...
def create_word_report(analysis_text, metadata):
    ...
    for row_idx, row_data in enumerate(info_data):
        ...
        for col_idx, text in enumerate(row_data):
            ...
            p = cell.paragraphs[0]
            run = p.add_run(text)
            set_font_style(run, size=12, bold=(col_idx % 2 == 0)) # 標籤加粗
            
    doc.add_paragraph()
    doc.add_paragraph()

    # 3. 解析 Markdown 內容並寫入
    # 簡單 parser：處理標題 (#), 列表 (*), 表格 (|)
    ...
    for line in lines:
        ...
        if line.startswith("###"):
            ...
            set_font_style(run, size=14, bold=True, color=RGBColor(91, 124, 153)) # 莫蘭迪藍
        ...
        # 處理列表 (* ...)
            ...
            run = p.add_run(clean_text)
            # 簡單著色邏輯：若開頭是 🟢/🔴/🟡/⚠️，雖Word無emoji顏色，但可設文字色
        ...
        # 一般文字
            ...
            set_font_style(p.runs[0] if p.runs else p.add_run(line), size=12)
    ...
    for r in range(rows):
        for c in range(min(cols, len(data[r]))):
            ...
            for p in cell.paragraphs:
                for run in p.runs:
                    set_font_style(run, size=10) # 表格字稍小
            # 第一列加強
            if r == 0:
                # 這裡略過底色設定，僅加粗
                for p in cell.paragraphs:
    ...
    with col2:
        ...
        with st.expander("⚠️ 使用前請務必詳閱免責聲明 (點擊展開)", expanded=True):
            ...
            **使用前請詳閱以下說明：**
            ...
            3. **資料留存規範**：本系統不永久留存檔案，上傳之文件將於系統重啟後自動銷毀。
...
with st.container():
    ...
    with col_dash_1:
        ...
        <div class="dashboard-card">
            ...
            <small>啟用強制分流：數學/自然 ➔ Gemini 3.0 Pro | 文科 ➔ Gemini 3.0 Flash</small>
        </div>
        """, unsafe_allow_html=True)
    with col_dash_2:
        with st.expander("⚙️ 手動模型設定"):
            model_mode = st.radio("模式", ["自動強制分流", "手動指定"], label_visibility="collapsed")
            if model_mode == "手動指定":
                # 這裡簡單列出兩個選項給老師選
...
if st.button("🚀 開始全方位審查", type="primary", use_container_width=True):
    if not exam_file:
        ...
        try:
            # ----------------------------------------------------
            # Phase 1: 極速掃描 (Metadata Extraction & Routing)
            # ----------------------------------------------------
            status_box.info("🔍 Phase 1: 正在使用 Flash 進行極速掃描與科目偵測...")
            ...
            如果找不到某些資訊，請填入空白。
            同時，請判斷這份試卷的主要科目類型，只回答：'MATH' (數學/自然/物理/化學) 或 'OTHER' (國語/英語/社會/生活)。
            """
            ...
            # 判斷科目類型 (用於路由)
            is_science = False
            if "MATH" in meta_response.text or any(s in metadata.get("subject", "") for s in ["數", "自然", "理", "化"]):
                is_science = True

            st.session_state.metadata = metadata # 存起來給 Word 用
            ...
            if model_mode == "手動指定":
                ...
                if is_science:
                    ...
                    routing_msg = "📐 偵測到理科試卷，強制切換 Gemini 3.0 Pro"
                else:
                    target_model_name = get_best_flash_model(api_key)
                    routing_msg = "📚 偵測到文科試卷，切換 Gemini 3.0 Flash"
...
1. **嚴禁開場白**：禁止輸出「依據剝皮測試標準...」等引言。
2. **禁止頁碼**：題目敘述中**嚴禁提及**「第x頁」。
3. **強制分組與排序**：在 Step 2 與 Step 3，先列出 🟢(優點/真素養)，再列出 🔴(缺點/假素養)，最後 🟡(建議)。
4. **強制換行**：每一個燈號項目都必須是獨立的 Bullet Point。
...
* **情況 A (若有提供單元名稱 且 有上傳教材)：** 製作標準雙向細目表。
