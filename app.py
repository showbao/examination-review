...
# --- 內容解析 (強力標題抓取版) ---
    ...
    for line in lines:
        ...
        # 1. 預先清洗：暫時移除開頭的 * 或 - 以便偵測真正內容
        # 這樣即使 AI 輸出 "* ### 🟢" 或是 "* 🟢" 也能被正確識別
        ...
        if is_header:
            ...
            # 取得乾淨文字 (這裡會再次呼叫 clean_markdown_symbol 移除 # 和 *)
            final_text = clean_markdown_symbol(line)
            
            p = doc.add_paragraph()
            run = p.add_run(final_text)
            
            # 依據燈號給予樣式 (強制大字體 14pt~16pt)
        ...
        # --- 一般內文列表 ---
            ...
            # 強制轉為列表樣式 (List Bullet)，字體較小 (12pt)
